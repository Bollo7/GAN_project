...
gan_hyps.add_argument('--BATCH_SIZE', type=int, default=64, help='The size of batch')
...
gan_hyps.add_argument('--N_EPOCHS', type=int, default=5, help='The number of epochs to run')
...
#gan_hyps.add_argument('--SEED', type=int, default=42, help='Seed of a current run')
	...
	for k in args.SEEDS:
		for i in args.LR_Ds:
			for j in args.LR_Gs:
				...
				save_dir, result_dir, log_dir = f'/system/user/kolesnic/GANS_server/{args.hyper_dir}/{save_dir}', f'/system/user/kolesnic/GANS_server/{args.hyper_dir}/{result_dir}', f'/system/user/kolesnic/GANS_server/{args.hyper_dir}/{log_dir}'
				...
				print(f'# # # Training with LR_G = {str(j)}, LR_D = {str(i)} # # #')
				gan.train()
				gan.save()
